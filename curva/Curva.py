...
class Curva:
    ...
    def interpolacion(s,p:int=0,r:float=0.0) -> list:
       ...
       if i == s.n-2:
        ...
        xi.append(zp1*s.x[ip1]+z*s.x[i]+zp2*s.x[im1])
        for j in range(1,s.dim):
        ...
        #===========================
           ...
           if i ==s.n-1:
            ...
            if i == 1:
                im2 = s.n-1
            i12:np.float64 = 1.0/12.0
            am1:np.float64 = a+1.0
            am2:np.float64 = a+2.0
            ap1:np.float64 = 1.0-a
            ap2:np.float64 = 2.0-a
            ap3:np.float64 = 3.0-a
            z:np.float64 = 1.0+a*a*u12*(-15.0+a*(-35.0+a*(-25.0))))
            zp1:np.float64 = 1.0+ap1*ap1*u12*(-15.0+ap1*(63.0+ap1*(-25))))
            zp2:np.float64 = -4.0+u12*ap2*(225.0+ap2*(-367.5+ap2*(-94.5+12.5*ap2))))
            zp3:np.float64 = 18.0+u12*ap3*(-459.0+ap3*(382.5+ap3*(-156.5+ap3*(31.5-2.5*ap3))))
            zm1:np.float64 = -4.0+u12*am1*(225.0+am1*(-367.5+am1*(-94.5+12.5*am1))))
            zm2:np.float64 = 18.0+u12*am2*(-459.0+am2*(382.5+am2*(31.5-2.5*am2))))
...
#==============================================================================
# Funxión zspline crea el Z-spline de un conjunto de puntos en dim dimensiones,
